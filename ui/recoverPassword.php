...
if(isset($_POST['recover'])){
	...
	if(!$foundEmail){
		$recoverProfesor = new Profesor();
		if($recoverProfesor -> existEmail($_POST['email'])) {;
			$generatedPassword = rand(100000,999999);
			$recoverProfesor -> recoverPassword($_POST['email'], $generatedPassword);
		$foundEmail = true;
		}
	}
	if(!$foundEmail){
		$recoverSecretaria = new Secretaria();
		if($recoverSecretaria -> existEmail($_POST['email'])) {;
			$generatedPassword = rand(100000,999999);
			$recoverSecretaria -> recoverPassword($_POST['email'], $generatedPassword);
		$foundEmail = true;
		}
	}
	if($foundEmail){
		$to=$_POST['email'];
		$subject="Password recovery for GBS";
		$from="FROM: GBS <contact@itiud.org>";
